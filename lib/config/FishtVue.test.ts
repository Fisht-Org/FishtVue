...
describe("Testing config", () => {
  ...
  describe("getStyle via global properties", () => {
    let app: App
    ...
    it("should return global styles when plugin is installed", () => {
      const options: FishtVueConfiguration = {
        optionsTheme: {
          nameTheme: "Aurora"
        },
      ...
      expect(fishtVueInstance).toBeDefined()
      expect(fishtVueInstance.getStyle).toBeDefined()

      // Вызов getStyle через глобальные свойства
      const stylesFromGlobal = fishtVueInstance.getStyle()
      expect(stylesFromGlobal).toBeDefined()
      expect(stylesFromGlobal.components.FixWindow).toBeDefined()
      expect(stylesFromGlobal.components.FixWindow.body.padding).toBe("0 4px 0 4px")
    })

    it("should return specific component styles when component name is provided", () => {
      const options: FishtVueConfiguration = {
        optionsTheme: {
          nameTheme: "Aurora"
        },
      ...
      // Вызов getStyle с именем компонента
      const fixWindowStyles = fishtVueInstance.getStyle("FixWindow")
      expect(fixWindowStyles).toBeDefined()
      // @ts-ignore
      expect(fixWindowStyles?.body?.padding).toBe("0 4px 0 4px")
    })
    ...
    it("should warn and return undefined when FishtVue plugin is not installed", () => {
      ...
      if (fishtVueInstance && fishtVueInstance.getStyle) {
        const stylesFromGlobal = fishtVueInstance.getStyle()
        expect(stylesFromGlobal).toBeUndefined()
        expect(warnSpy).toHaveBeenCalledWith("FishtVue is not installed!")
      }
